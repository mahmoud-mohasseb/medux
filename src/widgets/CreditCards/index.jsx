      ...
      <Navigation ref={headerRef}>
        <div className="tabs">
          <TabNav>
            <Item>
              <Button
                className={isActiveCards ? "active" : ""}
      ...
      {isActiveCards ? (
        <>
          <ScrollContainer height={contentHeight}>
            <Container
              style={{ overflowY: !formVisible ? "auto" : "hidden" }}
              ref={contentRef}
            >
          ...
          <Footer ref={footerRef}>
            ...
            <div className="btn-wrapper">
              <Btn
                text="Add new card"
                handler={handleShow}
                className={formVisible ? "disabled" : ""}
              />
            </div>
          </Footer>
        </>
        ...
        <Container>
          {archivedCards.length !== 0 ? (
            archivedCards.map(drawCards)
          ) : (
            <NoDataPlaceholder variant="cards" />
          )}
        </Container>
      )}
    </Widget>
