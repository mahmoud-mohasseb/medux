// styled components
import { MonthSelector, CalendarWrapper, StyledCalendar } from "./style";
...
const TaskScheduler = () => {
  ...
  const calendarStart = moment()
      .month(selectedRange.start - 1)
      .startOf("month")
      .toDate(),
    calendarEnd = moment()
      .month(selectedRange.end - 1)
      .endOf("month")
      .toDate();

  const docTasks = scheduler.filter(
    (task) => task.id === selectedDoctor.value
  )[0].tasks;

  const hasTasks = (date) =>
    docTasks.filter((event) => moment(event.date).isSame(date, "day")).length >
    0;

  const drawTasks = (date) => {
    const tasks = docTasks.filter((event) =>
          ...
          <div className="list">
            ...
            >
              {monthArray.map(({ month }) => {
                ...
                const isSelected =
                  +monthNumber === selectedRange.start ||
                  +monthNumber === selectedRange.end;
        ...
        <CalendarWrapper top={height}>
          ...
          >
            <div className={`popup ${open ? "visible" : ""}`}>
              ...
              {selectedDate &&
                hasTasks(selectedDate) &&
                drawTasks(selectedDate)}
            </div>
          </Backdrop>
        </CalendarWrapper>
        <div ref={footerRef}>
