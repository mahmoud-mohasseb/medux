...
const Payments = ({ arr, height }) => {
  const dates = [
    ...new Set(arr.map((item) => moment(item.date).format("YYYY-MM-DD"))),
      ...
      <PaymentItem>
        ...
        {item.additional && (
          <Additional>
            <div className="main">
              <Avatar
                avatar={item.additional.avatar}
                alt={item.additional.doctor}
              />
        ...
        <>
          {dates.map((date) => {
            ...
            return (
              <div key={nanoid()}>
                <GroupSeparator
                  text={date === moment().format("YYYY-MM-DD") ? "Today" : date}
                />
