// styled components
import { EventsCalendar, Container, Popup, Backdrop } from "./style";
...
const EventsCompactCalendar = () => {
  ...
  const hasEvents = (date) =>
    events.filter((event) => moment(event.date).isSame(date, "day")).length > 0;
  ...
  const config = {
    ...
    tileContent: ({ date }) =>
      hasEvents(date) ? <span className="bar" /> : null,
