// styled components
import { Rotate, BodyChart, View, Spots, Spot } from "../style";
...
const Body = ({ gender }) => {
  ...
  const { notify } = useNotistack(
    "Your request have been successfully sent to your doctor.",
    "info"
  );
  ...
  return (
    <div className="wrapper">
      <Rotate
        tabIndex="0"
        onClick={() => setFrontView((prev) => !prev)}
      ...
      >
        <div className="chart_content">
          {frontView ? (
            <View side="front">
              <img
                className={`${gender} front`}
                src={gender === "male" ? male_front : female_front}
                alt="Body chart"
              />
              <Spots>{drawSpots(frontAreas, "front", gender)}</Spots>
            </View>
          ) : (
            <View side="back">
              <img
                ...
                alt="Body chart"
              />
              <Spots>{drawSpots(backAreas, "back", gender)}</Spots>
            </View>
          )}
        </div>
        <Btn
          className={!isAnyChecked ? "disabled" : ""}
          text="Submit request"
          type="submit"
        />
      </BodyChart>
