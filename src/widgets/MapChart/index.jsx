...
const MapChart = () => {
  ...
  const diseases = [`${t("Ebola")}`, `${t("Coronavirus")}`, `${t("Malaria")}`];
  ...
  return (
    <Widget name="MapChart" mobile={300}>
      <WidgetNav
        title={`${diseases[index]} ${t("epidemic")}`}
        handler={navigate}
      />
      <WidgetBody>
        <MapChartWrapper>
          <div className="total">
            <CountUp
              formattingFn={numFormatter}
              end={currentData?.total}
              duration={2}
              className="num h1"
            />
            <span>{`${t("patients worldwide")}`}</span>
          </div>
